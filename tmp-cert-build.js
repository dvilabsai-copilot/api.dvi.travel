...
(async () => {
    ...
    hotel_bookings: [
      {
        ...
        roomType: roomName,
        checkInDate: String(row2.check_in_date).slice(0, 10),
        checkOutDate: String(row2.check_out_date).slice(0, 10),
        ...
        passengers: [
          {
            ...
            age: 30,
            phoneNo: '9876543210',
            pan: row2?.api_response?.persistenceSnapshot?.panDetails?.[0] || 'AAAPL1234C'
          },
          {
            title: 'Mrs',
            firstName: 'Test2',
            ...
            age: 28,
            phoneNo: '9876543211',
            pan: row2?.api_response?.persistenceSnapshot?.panDetails?.[1] || 'AFZPK7190K'
          }
        ]
      }
    ],
  ...
  const confirmResponse = {
    ...
    itinerary_plan_ID: row2.itinerary_plan_ID,
    confirmed_itinerary_plan_ID: row2.confirmed_itinerary_plan_ID,
    bookingResults: [
      {
        routeId: row2.itinerary_route_ID,
        hotelCode: row2.tbo_hotel_code,
        bookingId: row2.tbo_booking_id,
        status: 'confirmed',
        preBook: row2?.api_response?.preBookMeta || null,
        priceChanged: false,
        priceChangedMessage: null,
        mandatorySupplements: row2?.api_response?.preBookMeta?.mandatorySupplements || [],
        confirmation: {
          tbo_hotel_booking_confirmation_ID: row2.tbo_hotel_booking_confirmation_ID,
        },
      },
    ],
  };

  fs.writeFileSync('tmp-cert-db-row1.json', JSON.stringify(row1, replacer, 2));
  fs.writeFileSync('tmp-cert-db-row2.json', JSON.stringify(row2, replacer, 2));
  fs.writeFileSync('tmp-cert-confirm-request.json', JSON.stringify(confirmRequest, null, 2));
  fs.writeFileSync('tmp-cert-confirm-response.json', JSON.stringify(confirmResponse, null, 2));
