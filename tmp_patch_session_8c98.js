...
async function main() {
  ...
  const targetSession = '8c98cc4448a15aa1';
  ...
  if (!hotspot || !vehicle) throw new Error('Missing valid hotspot/vehicle records');

  const token = String(hotspot.hotspot_location).split('|').map(s => s.trim()).find(Boolean) || String(hotspot.hotspot_location);

  const rows = await prisma.dvi_tempcsv.findMany({
    where: { csvtype: 4, sessionID: targetSession },
    orderBy: { temp_id: 'asc' },
    select: { temp_id: true, field4: true, status: true, field5: true },
  });

  if (!rows.length) throw new Error(`No rows for session ${targetSession}`);
  ...
  console.log(JSON.stringify({ session: targetSession, hotspot: hotspot.hotspot_name, locationToken: token, vehicle: vehicle.vehicle_type_title, patched }, null, 2));
