 ...
 */
 ...
 this.logger.warn(` Could not update ${cityMap.dviCityName}: ${error}`);
        }
      }
 ...
 */
 ...
 this.logger.warn(` Could not fetch from API, using fallback codes for city ${tboCityCode}`);
        hotelCodes = this.FALLBACK_HOTELS[tboCityCode] || [];
      }

      if (hotelCodes.length === 0) {
 this.logger.warn(` No hotel codes available for city ${tboCityCode}`);
        return 0;
      }
 ...
 this.logger.warn(` Failed to upsert hotel ${hotelCode}: ${upsertError.message}`);
        }
 ...
 */
 ...
 this.logger.warn(` Could not fetch from TBO API: ${error.message}`);
      return [];
    }
  }
 ...
 */
 ...
 this.logger.debug(` Could not fetch details for hotel ${hotelCode}: ${error.message}`);
    }
