 ...
 */
 ...
 // Check if cityCode is numeric (TBO format) - if so, look up the city name
      if (/^\d+$/.test(criteria.cityCode)) {
        ...
        if (city) {
          cityName = city.name;
 this.logger.log(` Resolved TBO code ${criteria.cityCode} ${cityName}`);
        } else {
 this.logger.warn(` Could not resolve TBO city code: ${criteria.cityCode}, treating as city name`);
        }
      } else {
 this.logger.log(` Using city name directly: ${cityName}`);
      }
 ...
 */
 ...
 this.logger.warn(` Hotel ${hotelCode}: No property details available`);
        return null;
      }
 ...
 */
 ...
 this.logger.log(`\n Step 3 : Sending Booking Push to ResAvenue...`);
