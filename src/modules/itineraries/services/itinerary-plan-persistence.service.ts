...
function normalizeRouteDate(value: unknown): string {
  ...
  if (!raw) return '';
  // Route dates represent calendar days, not instants. Preserve the date
  // supplied by the form before parsing timezone-bearing ISO values; otherwise
  // midnight in a positive offset can compare as the previous UTC day.
  const calendarDate = raw.match(/^(\d{4}-\d{2}-\d{2})/);
  if (calendarDate?.[1]) return calendarDate[1];
...
function normalizeRouteNumber(value: unknown): string {
...
function routeShapeSignature(route: any): string {
  const viaRoutes = Array.isArray(route?.via_routes)
    ? route.via_routes.map((viaRoute: any) => [
      normalizeRouteNumber(viaRoute?.itinerary_via_location_ID ?? viaRoute?.locationId ?? viaRoute?.id),
      normalizeRouteText(
        viaRoute?.itinerary_via_location_name ??
        viaRoute?.via_route_name ??
        viaRoute?.location_name ??
        viaRoute?.name,
      ),
    ].join(':')).join('~')
    : '';
  ...
  return [
    normalizeRouteDate(route?.itinerary_route_date ?? route?.date),
    normalizeRouteText(route?.location_name ?? route?.source),
    normalizeRouteText(route?.next_visiting_location ?? route?.destination ?? route?.next),
    normalizeRouteNumber(route?.direct_to_next_visiting_place ?? route?.directVisit),
    normalizeRouteText(route?.via_route ?? route?.via),
    viaRoutes,
  ].join('|');
}

function planRouteShapeSignature(plan: any): string {
  return [
    normalizeRouteDate(plan?.trip_start_date ?? plan?.trip_start_date_and_time),
    normalizeRouteDate(plan?.trip_end_date ?? plan?.trip_end_date_and_time),
    normalizeRouteNumber(plan?.no_of_nights),
    normalizeRouteNumber(plan?.no_of_days),
    normalizeRouteText(plan?.arrival_point ?? plan?.arrival_location),
    normalizeRouteText(plan?.departure_point ?? plan?.departure_location),
  ].join('|');
}

/**
 * Route IDs are regenerated by the edit rebuild, so they cannot be used to
 * decide whether the hotel snapshot is still valid. Compare the semantic
 * route/stay shape instead. This is exported so the reset trigger has a
 * deterministic unit-testable contract.
 */
export function hasItineraryRouteChanged(
  previousRoutes: any[] = [],
  nextRoutes: any[] = [],
  previousPlan?: any,
  nextPlan?: any,
): boolean {
  const previousShape = previousRoutes.map(routeShapeSignature);
  const nextShape = nextRoutes.map(routeShapeSignature);

  if (previousShape.length !== nextShape.length) return true;
  if (previousShape.some((value, index) => value !== nextShape[index])) return true;
...
stepStartedAt = this.logItineraryApiTiming({
 ...
 // This ensures that when we rebuild hotspots later, we know which day each "tombstone" (deleted hotspot) belonged to.
      ...
      const oldViaRoutes = await (tx as any).dvi_itinerary_via_route_details.findMany({
        where: {
          itinerary_plan_ID: planId,
          status: 1,
          deleted: 0,
      ...
      const oldViaRoutesByRouteId = new Map<number, any[]>();
      for (const viaRoute of oldViaRoutes) {
        const routeId = Number(viaRoute?.itinerary_route_ID || 0);
        if (!routeId) continue;
        const existing = oldViaRoutesByRouteId.get(routeId) || [];
        existing.push(viaRoute);
        oldViaRoutesByRouteId.set(routeId, existing);
      }
      const oldRoutesWithViaRoutes = oldRoutes.map((route: any) => ({
        ...route,
        via_routes: oldViaRoutesByRouteId.get(Number(route?.itinerary_route_ID || 0)) || [],
      }));
      const oldRouteDateMap = new Map(oldRoutes.map((r: any) => [r.itinerary_route_ID, r.itinerary_route_date]));

      // Decide whether this is a route edit before deleting/recreating route
      // rows. Non-route edits must preserve route IDs, hotel detail rows, and
      // the persisted supplier snapshot selected by the user.
      routeChanged = isPlanUpdate && hasItineraryRouteChanged(
        oldRoutesWithViaRoutes,
        Array.isArray(dto.routes) ? dto.routes : [],
        previousRoutePlan,
        dto.plan,
      );
...
const shouldRebuildTimeline =
  shouldRebuildRouteData || itineraryStartTimeChanged;
      ...
      if (isPlanUpdate && !routeChanged && !roomCountChanged && !mealPlanChanged) {
        console.log('[HOTEL_ROUTE_CHANGE_NOT_DETECTED]', {
          ...
          preservedHotelSnapshot: true,
        });
      }

      if (shouldResetManualHotspotsForFullRebuild && shouldRebuildRouteData) {
 ...
 // Clear old route-linked rows before deleting/recreating routes to avoid update 500s.
      if (shouldRebuildRouteData && isPlanUpdate) {
      ...
      const routes = shouldRebuildRouteData
        ? await this.routeEngine.rebuildRoutes(
            planId,
            dto.plan,
            dto.routes,
            tx,
...
if (!shouldRebuildRouteData && itineraryStartTimeChanged) {
 ...
 console.log(shouldRebuildRouteData ? '[PERF] rebuildRoutes:' : '[PERF] preserveRoutes:', Date.now() - opStart2, 'ms');
 ...
 console.log('[PERF] rebuildViaRoutes:', Date.now() - opStart2, 'ms');
        ...
        // Via routes are part of the permit location chain, so permits must
        // be rebuilt after the via-route rows have been persisted.
        opStart2 = Date.now();
