...
export class ItinerariesService {
  ...
  constructor(
    ...
    this.hotelBookingFulfillmentService.setCallbacks({
      ...
      getConfirmedItineraryDetails: (...args) => (this.getConfirmedItineraryDetails as any)(...args),
      mergeConsecutiveSupplierHotelBookings: (...args) => (this.confirmationService.mergeConsecutiveSupplierHotelBookings as any)(...args),
      pruneHotelBookingsCoveredByMultiNight: (...args) => (this.confirmationService.pruneHotelBookingsCoveredByMultiNight as any)(...args),
      getProviderBookableHotelBookings: (...args) => (this.confirmationService.getProviderBookableHotelBookings as any)(...args),
    });
    ...
    this.manualHotspotPreviewService.setCallbacks({
      ...
      runManualHotspotBatchWithinTransaction: (...args) => (this.runManualHotspotBatchWithinTransaction as any)(...args),
    });
    ...
    this.exactAnchorRebuildService.setCallbacks({
      ...
      buildManualFitTravelReplicaDisplayFields: (...args) => (this.buildManualFitTravelReplicaDisplayFields as any)(...args),
      chooseReliableTravelDistanceKm: (...args) => (this.chooseReliableTravelDistanceKm as any)(...args),
      classifyManualHotspotCityContext: (...args) => (this.classifyManualHotspotCityContext as any)(...args),
...
async getAvailableActivities(hotspotId: number, planId?: number, routeId?: number) {
  ...
  * - already added hotspots => exclude from addable list for this route/day
  ...
  private buildProgressiveRemovalReason(
  ...
  private buildSelectedClosingRemovalReason(params: {
    removedName: string;
  ...
  private buildProgressiveRemovalSuccessMessage(
    validationMode: 'DAY_END' | 'SELECTED_HOTSPOT_CLOSING',
  ...
  private async getActiveRouteHotspotIdSetInTx(
    tx: any,
    planId: number,
    routeId: number,
  ): Promise<Set<number>> {
  ...
  private async getActiveRouteManualFitRemovalEvidenceInTx(
    tx: any,
    planId: number,
    routeId: number,
  ): Promise<{
  ...
  private async getActiveRouteManualFitRemovalEvidence(
    planId: number,
    routeId: number,
  ): Promise<{
    activeHotspotIds: Set<number>;
  ...
  private buildManualFitFinalizedPreviewTimeline(
    timeline: any[],
    removedRows: any[],
  ): any[] {
    return buildManualFitFinalizedPreviewTimelineImpl.call(this, timeline, removedRows);
  }

  private buildManualFitAttemptTimelineSnapshot(
    timeline: any[],
    params: {
      removedHotspotIds?: number[];
      selectedHotspotId?: number;
    ...
    return buildManualFitAttemptTimelineSnapshotImpl.call(this, timeline, params);
  }

  private buildManualFitAttemptDisplayTimelineSnapshot(
    ...
    return buildManualFitAttemptDisplayTimelineSnapshotImpl.call(this, displaySourceTimeline, params);
  }

  private buildManualFitAttemptComputedDisplayTimelineSnapshot(
    ...
    return buildManualFitAttemptComputedDisplayTimelineSnapshotImpl.call(
      this,
  ...
  private validateManualFitAttemptDisplayTimeline(
    rows: any[],
    params: {
      removedHotspotIds: number[];
      selectedHotspotId: number;
    ...
    return validateManualFitAttemptDisplayTimelineImpl.call(this, rows, params);
  }

  private async filterPlannedRemovalsToSameRouteInTx(
    tx: any,
    planId: number,
    routeId: number,
    removals: any[],
  ...
  private async resolveProgressivePriorityRemovalForManualFitInTx(
    tx: any,
    params: {
      planId: number;
      routeId: number;
    ...
    const isAttractionLikeRow = (row: any): boolean => {
      if (this.isAttractionTimelineRow(row)) return true;
    ...
    if (params.exactAnchorMode === true && sourceTimelineAttractionCount === 0) {
      const fallbackRouteTimeline = await this.getRouteTimelineForScoring(
        tx,
    ...
    const removalSourceCityKey = this.deriveLooseCityKey(removalRouteCityContext.location_name);
    const removalDestinationCityKey = this.deriveLooseCityKey(removalRouteCityContext.next_visiting_location);
    ...
    const getRemovalCityContext = (hotspotId: number, row: any = null): ManualHotspotCityContext => {
      ...
      return this.classifyManualHotspotCityContext(removalRouteCityContext, {
      ...
      .map(({ row, rowIndex }: any) => {
        ...
        return {
          ...
          estimatedMinutes: Number(
            ...
            this.getHotspotDurationMinutes(master, row) ||
            this.getPreviewRowDurationMinutes(row) ||
            0,
          ),
          endMinutes: this.parseSegmentEndMinutes(row) ?? 0,
        };
      })
      .sort((a: any, b: any) => {
      ...
      candidateRows = (activeRouteRows || [])
        .map((row: any, rowIndex: number) => {
          ...
          return {
            ...
            priority,
            estimatedMinutes: Number(
              this.getHotspotDurationMinutes(master, row) || 0,
            ),
            endMinutes: row?.hotspot_end_time
              ? Math.floor(
                  this.hmsToSeconds(TimeConverter.toTimeString(row.hotspot_end_time)) / 60,
                )
              : 0,
    ...
    const evaluateTimeline = async (timeline: any[]) => {
      ...
      if (validationMode === 'SELECTED_HOTSPOT_CLOSING') {
        evaluatedTimeline = await this.enrichManualFitPreviewTimelineWithOperatingHours(
          Number(params.planId),
          Number(params.routeId),
          evaluatedTimeline,
        );
      ...
      const selectedTargetEndMinutes =
        selectedTargetRow ? this.parseSegmentEndMinutes(selectedTargetRow) : null;

      let selectedClosingOverflowMinutes = 0;
      let selectedOpeningConflict: any | null = null;

      if (validationMode === 'SELECTED_HOTSPOT_CLOSING') {
        const selectedClosingCheck = this.getSelectedManualClosingOverflow({
          timeline: evaluatedTimeline,
          selectedHotspotIds: [targetHotspotId],
        });

        const selectedOperatingValidation = this.markSelectedManualOperatingHourConflicts(
          evaluatedTimeline,
          [targetHotspotId],
        );
        ...
        return {
          ...
          finalArrivalTime:
            selectedTargetEndMinutes
              ? this.minutesRangeToFitPreviewLabel(selectedTargetEndMinutes, selectedTargetEndMinutes)
              : (maxEndMinutes > 0 ? this.minutesRangeToFitPreviewLabel(maxEndMinutes, maxEndMinutes) : null),
          evaluatedTimeline,
      ...
      return {
        ...
        selectedTargetRow,
        finalArrivalTime:
          maxEndMinutes > 0 ? this.minutesRangeToTimeString(maxEndMinutes, maxEndMinutes) : null,
        evaluatedTimeline,
    ...
    const rebuildAfterRemoval = async (removedIds: number[]) => {
      ...
      if (validationMode === 'SELECTED_HOTSPOT_CLOSING' || params.exactAnchorMode === true) {
        return this.buildExactAnchorSequentialTimelineAfterRemoval(tx, sourceTimeline, {
      ...
      return this.buildMatrixRouteTimelineAfterLowPriorityRemoval(
        tx,
        sourceTimeline,
        removedIds,
        {
    ...
    if (validationMode === 'SELECTED_HOTSPOT_CLOSING') {
      ...
      if (!sourceContainsTarget) {
        const baselineExactTimeline = await this.buildExactAnchorSequentialTimelineAfterRemoval(tx, sourceTimeline, {
          removedHotspotIds: [],
          targetHotspotId,
          routeId: Number(params.routeId),
          planId: Number(params.planId),
    ...
    const toRemovedRows = (
      ...
      const reason =
        validationMode === 'SELECTED_HOTSPOT_CLOSING'
          ? this.buildSelectedClosingRemovalReason({
          ...
          : this.buildProgressiveRemovalReason(validationMode, row.priority);
    ...
    if (preselectedRemovalIds.length > 0) {
      ...
      const attemptComputedTimeline = this.buildManualFitAttemptTimelineSnapshot(workingTimeline, {
        removedHotspotIds: selectedRemovedIds,
        selectedHotspotId: targetHotspotId,
      });
      const attemptDisplayTimeline = this.buildManualFitAttemptComputedDisplayTimelineSnapshot(
        sourceTimeline,
        workingTimeline,
        {
          removedHotspotIds: selectedRemovedIds,
      ...
      const displayTimelineErrors = this.validateManualFitAttemptDisplayTimeline(attemptDisplayTimeline, {
        removedHotspotIds: selectedRemovedIds,
        selectedHotspotId: targetHotspotId,
      });
      ...
      if (evaluation.valid) {
        return {
          ...
          rejectedAttempts,
          candidateAudit,
          message: this.buildProgressiveRemovalSuccessMessage(validationMode, selectedRemovedRows),
        };
      }
    }
    ...
    if (validationMode === 'SELECTED_HOTSPOT_CLOSING' && params.exactAnchorMode === true && candidateRows.length > 0) {
      ...
      for (const plan of rescuePlans) {
        ...
        const attemptComputedTimeline = this.buildManualFitAttemptTimelineSnapshot(planTimeline, {
        ...
        if (evaluation.valid || selectedClosingPlanResolved) {
          return {
            ...
            rejectedAttempts,
            candidateAudit,
            message: this.buildProgressiveRemovalSuccessMessage(validationMode, planRows),
          };
        }
    ...
    for (const priority of [4, 3, 2, 1]) {
      ...
      for (const candidate of rowsForPriority) {
        ...
        const attemptComputedTimeline = this.buildManualFitAttemptTimelineSnapshot(nextTimeline, {
        ...
        if (evaluation.valid) {
          return {
            ...
            rejectedAttempts,
            candidateAudit,
            message: this.buildProgressiveRemovalSuccessMessage(validationMode, nextRemovedRows),
          };
        }
  ...
  private destinationSidePreviewDroppedBaselineRows(params: {
  ...
  private async pruneManualFitBacktrackingAfterSelectedPivotInTx(
    tx: any,
    params: {
      routeId: number;
      timeline: any[];
  ...
  private rebuildDestinationSidePreviewFromBaseline(params: {
    baselineTimeline: any[];
    manualInsertionFit: any;
    selectedHotspotId: number;
    hotspotMasters: any[];
  ...
  private chooseReliableTravelDistanceKm(
  ...
  private async buildMatrixRescheduledPreviewTimeline(params: {
    ...
    if (!fromHotspotId && toHotspotId > 0) {
      ...
      if (toRowIndex >= 0 && insertedRowIndex >= 0 && tx && Number(routeId || 0) > 0) {
        ...
        const sourceLeg = await this.resolveSavedRuleSourceToHotspotLeg(tx, Number(routeId || 0), selectedIdNum);
        const selectedToAnchorLeg = await this.resolveSavedRuleHotspotToHotspotLeg(tx, selectedIdNum, toHotspotId);
        ...
        for (const row of pendingHotelTravelRows) {
          ...
          const duration = Math.max(
            1,
            Math.round(Number(savedHotelLeg?.durationMin || this.getPreviewRowDurationMinutes(row) || row?.matrixDurationMin || 10)),
          );
          ...
          const hotelDistanceKm = this.chooseReliableTravelDistanceKm(
          ...
          rescheduledBody.push({
            ...
            matrixDurationMin: duration,
            duration: this.formatPreviewTravelDuration(duration),
            hotspot_start_time: null,
            hotspot_end_time: null,
          });
          cursor += duration;
    ...
    const savedAcLeg: any = tx
      ? await this.resolveSavedRuleHotspotToHotspotLeg(tx, fromHotspotId, selectedIdNum)
      : null;
    const savedCbLeg: any = tx
      ? (
          destinationSideHotelSlot || toHotspotId <= 0
            ...
            : await this.resolveSavedRuleHotspotToHotspotLeg(tx, selectedIdNum, toHotspotId)
        )
      : null;
    ...
    const acDistanceKm = this.chooseReliableTravelDistanceKm(
      savedAcLeg?.distanceKm != null ? Number(savedAcLeg.distanceKm) : null,
      bestSlot?.acOsrmDistanceKm != null ? Number(bestSlot.acOsrmDistanceKm) : null,
    );
    const cbDistanceKm = this.chooseReliableTravelDistanceKm(
  ...
  private async runManualHotspotBatchWithinTransaction(
    ...
    if (options?.forceConflictInsertion === true && options?.previewOnly !== true) {
      ...
      const refreshedTimeline = await this.buildRouteTimelineSnapshotAfterManualConflictInsert(
        tx,
        Number(planId),
        Number(routeId),
      );
    ...
    const adaptive = await this.runAdaptiveManualHotspotSetInsertion(
      ...
      },
      {
        ...
        afterHotspotId: options?.afterHotspotId,
        beforeHotspotId: options?.beforeHotspotId,
        anchorLabel: this.buildManualFitAnchorLabel({
          anchorType: resolvedAnchorType,
          anchorIndex: resolvedAnchorIndex,
    ...
    const authoritativeRemovedHotspots = this.getAuthoritativeManualFitRemovedHotspots({
      selectedAttempt,
    ...
    adjustedPreviewTimeline = this.removeManualFitDroppedRowsFromTimeline(
    ...
    if (
      ...
      destinationExactAnchorRebuilt = this.removeManualFitDroppedRowsFromTimeline(
    ...
    const removedPreviewHotspotIds = removedPreviewHotspots
      .map((row: any) => Number(this.getManualFitRemovalHotspotId(row) || 0))
      .filter((id: number) => Number.isFinite(id) && id > 0);
    ...
    const destinationPreviewDroppedBaselineRows =
      ...
      && this.destinationSidePreviewDroppedBaselineRows({
        baselineTimeline: baselineTimelineForMatrix,
    ...
    if (destinationPreviewDroppedBaselineRows) {
      let destinationBaselineRebuilt = await this.buildExactAnchorSequentialTimelineAfterRemoval(tx, baselineTimelineForMatrix, {
      ...
      if (this.isManualPreviewTimelineWrapped(destinationBaselineRebuilt)) {
      ...
      if (destinationBaselineRebuilt.length === 0) {
        destinationBaselineRebuilt = this.rebuildDestinationSidePreviewFromBaseline({
          baselineTimeline: baselineTimelineForMatrix,
          manualInsertionFit: timelineInsertionFit,
          selectedHotspotId: Number(focusHotspotId),
          hotspotMasters,
    ...
    if (
      ...
      const exactAnchorRescueSourceTimeline =
        Array.isArray(baselineTimelineForMatrix) && baselineTimelineForMatrix.length > 0
          ? baselineTimelineForMatrix
          : adjustedPreviewTimeline;
      const exactAnchorRemovalPlan = await this.resolveProgressivePriorityRemovalForManualFitInTx(tx, {
        planId: Number(planId),
        routeId: Number(routeId),
        selectedHotspotId: Number(focusHotspotId),
        selectedManualPriority,
      ...
      const sameRouteExactAnchorRemovals = await this.filterPlannedRemovalsToSameRouteInTx(
        tx,
        Number(planId),
        Number(routeId),
      ...
      const resolvedExactAnchorRemovals =
        sameRouteExactAnchorRemovals.length > 0
          ...
          : ((exactAnchorRemovalPlan.removedHotspots || []).filter((row: any) => this.getManualFitRemovalHotspotId(row) > 0));
      const sameRouteExactAnchorCandidates = await this.filterPlannedRemovalsToSameRouteInTx(
        tx,
        Number(planId),
        Number(routeId),
      ...
      const exhaustiveExactAnchorCandidateRemovals = (() => {
        const fallbackCandidates = (exactAnchorRemovalPlan.candidateHotspots || []).filter((row: any) => (
          this.getManualFitRemovalHotspotId(row) > 0
        ));
      ...
      const exhaustiveExactAnchorTimeline = exhaustiveExactAnchorRemovalIds.length > 0
        ? await this.buildExactAnchorSequentialTimelineAfterRemoval(tx, exactAnchorRescueSourceTimeline, {
    ...
    let finalValidationBase = this.buildManualHotspotValidation({
      route,
      requestedHotspotIds,
    ...
    if (shouldRunLowPriorityOverflowResolver) {
      ...
      const lowPriorityRemovalPlanPreview = await this.resolveProgressivePriorityRemovalForManualFitInTx(tx, {
        planId: Number(planId),
        routeId: Number(routeId),
        selectedHotspotId: Number(focusHotspotId),
        selectedManualPriority,
      ...
      const sameRouteDayEndResolvedRemovals = await this.filterPlannedRemovalsToSameRouteInTx(
        tx,
        Number(planId),
        Number(routeId),
      ...
      const resolvedDayEndRemovals =
        sameRouteDayEndResolvedRemovals.length > 0
          ...
          : ((lowPriorityRemovalPlanPreview.removedHotspots || []).filter((row: any) => this.getManualFitRemovalHotspotId(row) > 0));
    ...
    adjustedPreviewTimeline = await this.enrichManualFitPreviewTimelineWithOperatingHours(
      Number(planId),
      Number(routeId),
      adjustedPreviewTimeline,
    );
    ...
    const pivotBacktrackingPrune = await this.pruneManualFitBacktrackingAfterSelectedPivotInTx(tx, {
      routeId: Number(routeId),
      timeline: adjustedPreviewTimeline,
      selectedHotspotId: Number(focusHotspotId),
    });
    ...
    if (pivotBacktrackingPrune.removedHotspots.length > 0) {
      ...
      if (newBacktrackingRemovals.length > 0) {
        ...
        const pivotRebuiltTimeline = await this.buildExactAnchorSequentialTimelineAfterRemoval(tx, baselineTimelineForMatrix, {
    ...
    const tryDirectClickedAnchorClosingRescue = async (): Promise<boolean> => {
      ...
      for (const removedHotspotIds of rescuePlans) {
        ...
        const directRescueRemovals = removedHotspotIds.map((removedId: number) => {
          ...
          const reason = this.buildSelectedClosingRemovalReason({
            removedName,
            selectedHotspotLabel,
            attemptedVisitTime:
    ...
    if (shouldRunSelectedClosingResolver && !directClickedAnchorClosingRescueApplied) {
      ...
      const openingHoursRemovalPlan = await this.resolveProgressivePriorityRemovalForManualFitInTx(tx, {
        planId: Number(planId),
        routeId: Number(routeId),
        selectedHotspotId: Number(focusHotspotId),
        selectedManualPriority,
      ...
      const sameRouteResolvedRemovals = await this.filterPlannedRemovalsToSameRouteInTx(
        tx,
        Number(planId),
        Number(routeId),
      ...
      const resolvedOpeningHourRemovals =
        sameRouteResolvedRemovals.length > 0
          ...
          : ((openingHoursRemovalPlan.removedHotspots || []).filter((row: any) => this.getManualFitRemovalHotspotId(row) > 0));
      ...
      if (openingHoursRemovalPlan.resolved) {
        let finalResolvedTimeline = this.sanitizeResolvedLowPriorityTimeline(
        ...
        finalResolvedTimeline = await this.enrichManualFitPreviewTimelineWithOperatingHours(
          Number(planId),
          Number(routeId),
          finalResolvedTimeline,
        );

        const selectedAfterRescue = this.getSelectedManualClosingOverflow({
    ...
    let selectedOperatingValidation = this.markSelectedManualOperatingHourConflicts(
      adjustedPreviewTimeline,
      requestedHotspotIds,
    );
    ...
    finalValidationBase = this.buildManualHotspotValidation({
      route,
      requestedHotspotIds,
      fullTimeline: adjustedPreviewTimeline,
      manualTimingPolicy,
    ...
    const timingRisk = this.detectManualFitTimingRisk({
      timeline: adjustedPreviewTimeline,
      selectedHotspotId: Number(focusHotspotId),
    });
    ...
    const removedPrioritySummary = this.buildRemovedPrioritySummary([
      ...authoritativeRemovedHotspots,
      ...(adaptive.p3HotspotsToRemove || []),
    ]);
    ...
    adjustedPreviewTimeline = await this.ensurePreviewTimelineHasComputedHotelTravel(
      tx,
      Number(planId),
      Number(routeId),
      adjustedPreviewTimeline,
  ...
  private buildManualFitTravelReplicaDisplayFields(
    sourceRow: any,
    durationMin: number,
    distanceKm: number | null,
  ): {
    ...
    const durationText = durationCandidates
      .map((value: any) => {
        ...
        return this.formatPreviewTravelDuration(parsedMinutes);
      })
      .find((value) => String(value || '').trim().length > 0) || this.formatPreviewTravelDuration(durationMin);
  ...
  private async ensurePreviewTimelineHasComputedHotelTravel(
    ...
    const hotelNameHint = this.extractPreviewCheckinHotelName(originalCheckinRow);
    ...
    const savedHotelLeg = await this.resolveSavedRuleHotspotToRouteHotelLeg(
      tx,
      Number(planId),
      Number(routeId),
      anchorHotspotId,
    ...
    const anchorEndMinutes = this.parseSegmentEndMinutes(anchorRow);
    ...
    const computedTravelRow = {
      ...
      displayToName: hotelLabel,
      timeRange: this.minutesRangeToTimeString(travelStartMin, travelEndMin),
      duration: this.formatPreviewTravelDuration(durationMin),
      travelDuration: this.formatPreviewTravelDuration(durationMin),
      matrixDurationMin: durationMin,
    ...
    const computedCheckinRow = {
      ...
      toName: hotelLabel,
      to: hotelLabel,
      time: this.minutesRangeToTimeString(travelEndMin, travelEndMin),
      timeRange: this.minutesRangeToTimeString(travelEndMin, travelEndMin),
      hotspot_start_time: null,
      hotspot_end_time: null,
    };
    ...
    if (previousTravelIndex >= 0) {
      ...
      return this.finalizeMatrixPreviewTimeline(timeline);
    }
  ...
  private async resolveRouteSourceEndpoint(
    tx: any,
    routeId: number,
  ...
  private async resolveSourceToHotspotLeg(
    tx: any,
    routeId: number,
    hotspotId: number,
  ...
  private async buildManualInsertionFit(
    ...
    const attractionCityContexts = routeAttractions.map((row: any, index: number) => {
      ...
      const rowContext = this.classifyManualRouteAttractionCityContext(routeCityContext, {
    ...
    if (
      ...
      if (boundaryFromId > 0 && boundaryToId > 0 && !matrixBySlot.has(boundaryKey)) {
        const boundaryMx = await this.ensureRouteBetweenMapRow(
          tx,
          boundaryFromId,
          boundaryToId,
          Number(candidateHotspotId),
    ...
    if (
      ...
      if (boundaryFromId > 0 && boundaryToId > 0) {
        ...
        const directLeg = await this.getCachedRouteMatrixLeg(tx, boundaryFromId, boundaryToId);
        const acLeg = await this.getCachedRouteMatrixLeg(tx, boundaryFromId, Number(candidateHotspotId));
        const cbLeg = await this.getCachedRouteMatrixLeg(tx, Number(candidateHotspotId), boundaryToId);

        const directKm =
          directLeg?.distanceKm
          ?? this.distanceBetweenHotspots(masterById, boundaryFromId, boundaryToId)
          ?? null;
        const acDistanceKm =
          acLeg?.distanceKm
        ...
        const destinationBoundarySlot = {
          ...
          routeFitType,
          label: this.routeFitLabel(routeFitType),
          displayLabel: this.routeFitLabel(routeFitType),
          shortLabel: this.routeFitLabel(routeFitType),
          roadDetourKm,
          ...
          candidateDistanceFromAbRouteMeters: null,
          destinationDistanceFromAcRouteMeters: null,
          routePossible: this.isFeasibleFitType(routeFitType),
          timingPossible: timingEval.timingPossible,
          prioritySafe: true,
          selectedAsBest: false,
          attempted: true,
  ...
  private async runAdaptiveManualHotspotSetInsertion(
    ...
    const initialRun = await this.runManualClusterOptimizer(
      tx,
      Number(planId),
      Number(routeId),
      normalizedManualHotspotIds,
    ...
    const currentCandidatesRaw = await this.buildRouteHotspotInsertionCandidates(
      tx,
      Number(planId),
      Number(routeId),
      normalizedManualHotspotIds,
    ...
    const optionalCandidates = allowP3Removal
      ...
      .sort((a: any, b: any) => {
        const aPriority = this.getEffectivePriorityForManualInsertion(a);
        const bPriority = this.getEffectivePriorityForManualInsertion(b);
        const rankDiff = this.mapOptionalRemovalPriority(aPriority) - this.mapOptionalRemovalPriority(bPriority);
        if (rankDiff !== 0) return rankDiff;
    ...
    for (const candidate of optionalCandidates) {
      ...
      const explainedCandidate = this.enrichRemovedHotspotCandidateWithAttempt({
        candidate,
      ...
      const refreshedCandidatesRaw = await this.buildRouteHotspotInsertionCandidates(
        tx,
        Number(planId),
        Number(routeId),
        normalizedManualHotspotIds,
      ...
      const optionalRun = await this.runManualClusterOptimizer(
        tx,
        Number(planId),
        Number(routeId),
        normalizedManualHotspotIds,
      ...
      removedOptionalHotspots.push({
        ...this.buildRemovedHotspotExplanation({
          row: {
            ...explainedCandidate,
    ...
    const p3HotspotsToRemove = allowP3Removal
      ...
      .map((row: any) => {
        const explainedRow = this.enrichRemovedHotspotCandidateWithAttempt({
          candidate: row,
        ...
        return this.buildRemovedHotspotExplanation({
          row: explainedRow,
    ...
    for (const candidate of p3ConfirmedCandidates) {
      ...
      const refreshedCandidates = isPreviewOnly
        ? filterSimulatedRemovedCandidates(refreshedCandidatesRaw)
        : refreshedCandidatesRaw;
      const p3Run = await this.runManualClusterOptimizer(
        tx,
        Number(planId),
        Number(routeId),
        normalizedManualHotspotIds,
    ...
    if (shouldExploreProtectedPriorityRemovals) {
      ...
      for (const priorityLevel of removablePriorityOrder) {
        let priorityCandidatesStateRaw = await this.buildRouteHotspotInsertionCandidates(
          tx,
          Number(planId),
          Number(routeId),
          normalizedManualHotspotIds,
        ...
        for (const candidate of priorityCandidates) {
          ...
          priorityCandidatesStateRaw = await this.buildRouteHotspotInsertionCandidates(
            tx,
            Number(planId),
            Number(routeId),
            normalizedManualHotspotIds,
          ...
          priorityCandidatesState = isPreviewOnly
            ? filterSimulatedRemovedCandidates(priorityCandidatesStateRaw)
            : priorityCandidatesStateRaw;
          const priorityRun = await this.runManualClusterOptimizer(
            tx,
            Number(planId),
            Number(routeId),
            normalizedManualHotspotIds,
  ...
  async getConfirmedItineraryDetails(confirmedPlanId: number) {
    ...
    const guideAssignments = await this.listConfirmedGuideAssignments(confirmedPlanId);
