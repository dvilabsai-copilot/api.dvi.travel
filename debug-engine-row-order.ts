...
async function main() {
  ...
  for (const route of routes) {
    console.log(`\n--- ROUTE ID: ${route.itinerary_route_ID} ---`);
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    const typeName = typeNames[h.item_type] || `TYPE_${h.item_type}`;
    const startTime = h.hotspot_start_time ? new Date(h.hotspot_start_time).toISOString().substring(11, 19) : 'N/A';
    const route = routes.find((r: any) => r.itinerary_route_ID === h.itinerary_route_ID);
    
    console.log(`${i + 1}. [Route ${route?.itinerary_route_ID}] ${typeName.padEnd(15)} | ${startTime} | Order: ${h.hotspot_order} | HotspotID: ${h.hotspot_ID}`);
  }
}
