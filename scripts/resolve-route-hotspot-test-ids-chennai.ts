...
const ROUTE_SLOTS: RouteSlotInput[] = [
  { from: 'Government Museum Chennai', to: 'Mahabalipuram Group of Monuments' },
  { from: 'Mahabalipuram Group of Monuments', to: 'Promenade Beach' },
];
...
export async function resolveHotspotTestIdsChennai(): Promise<ResolveResult> {
...
async function main() {
  const result = await resolveHotspotTestIdsChennai();
...
main()
  .catch((error) => {
    console.error('Failed resolving Chennai route hotspot test IDs:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
