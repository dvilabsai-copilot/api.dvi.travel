...
async function main() {
  const rows = await prisma.$queryRaw`
    ...
    WHERE LOWER(TRIM(f.hotspot_name)) LIKE LOWER('%Eravikulam%')
      AND LOWER(TRIM(t.hotspot_name)) LIKE LOWER('%Rose Garden%')
  ...
  console.log('Verification: Pothamedu NOT between Eravikulam -> Rose Garden');
  console.log(`Rows found: ${rows.length}`);

  if (!rows.length) {
    console.log('Result: PASS (no feasible between-row, warn as OFF_ROUTE / deviation).');
    return;
  }

  console.log('Result: FAIL (unexpected feasible between-row found).');
