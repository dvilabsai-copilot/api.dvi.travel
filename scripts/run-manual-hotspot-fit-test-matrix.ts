...
import { resolveHotspotTestIds } from './resolve-route-hotspot-test-ids';
...
async function pothameduFocusedQuery() {
  const rows = await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
    SELECT
      f.hotspot_name AS from_name,
      t.hotspot_name AS to_name,
    ...
    JOIN dvi_hotspot_place t ON t.hotspot_ID = bm.to_hotspot_id
    JOIN dvi_hotspot_place c ON c.hotspot_ID = bm.between_hotspot_id
    WHERE bm.between_hotspot_id = 219
      AND (
        (bm.from_hotspot_id = 228 AND bm.to_hotspot_id = 218)
        OR
        (bm.from_hotspot_id = 218 AND bm.to_hotspot_id = 220)
      )
...
async function matrixStatusQuery() {
  const rows = await prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
    SELECT from_hotspot_id, to_hotspot_id, process_status, osrm_distance_km, osrm_duration_min
    FROM hotspot_route_matrix
    WHERE (from_hotspot_id = 228 AND to_hotspot_id = 218)
       OR (from_hotspot_id = 218 AND to_hotspot_id = 220)
...
async function main() {
  const resolved = await resolveHotspotTestIds();
  ...
  if (resolved.unresolvedOptionalNames.length) {
    console.log('Optional names missing and will be skipped:');
    for (const name of resolved.unresolvedOptionalNames) {
      console.log(`- ${name}`);
    }
  }
  const slots: Slot[] = [
    ...resolved.routeSlotsResolved.map((slot) => ({
      fromId: slot.from.hotspot_ID,
      toId: slot.to.hotspot_ID,
      fromName: slot.from.hotspot_name,
      toName: slot.to.hotspot_name,
    })),
    ...resolved.optionalHotelEdgeSlotsResolved.map((slot) => ({
      fromId: slot.from.hotspot_ID,
      toId: slot.to.hotspot_ID,
      fromName: slot.from.hotspot_name,
      toName: slot.to.hotspot_name,
  ...
  if (!resolved.optionalHotelEdgeSlotsResolved.length) {
    console.log('Matrix unavailable for hotel/source segment.');
  }
  ...
  const pothameduRows = await pothameduFocusedQuery();
  const matrixRows = await matrixStatusQuery();
  ...
  console.log('\nMatrix Status (focused pairs):');
  console.log(JSON.stringify(matrixRows, null, 2));

  console.log('\nSummary Query Rows:');
  console.log(JSON.stringify(summaryRows, null, 2));

  console.log('\nPothamedu Focused Rows:');
  console.log(JSON.stringify(pothameduRows, null, 2));
}

main()
  .catch((error) => {
    console.error('Failed running manual hotspot fit test matrix:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
