...
import { resolveHotspotTestIdsSouth } from './resolve-route-hotspot-test-ids-south';
...
async function poovarFocusedQuery(slots: Slot[], candidates: Candidate[]) {
  const poovar = candidates.find((item) => item.name.toLowerCase().includes('poovar island'));
  if (!poovar || !slots.length) return [];
  ...
  const rows = await prisma.$queryRawUnsafe<Array<Record<string, unknown>>>(`
    ...
    JOIN dvi_hotspot_place t ON t.hotspot_ID = bm.to_hotspot_id
    JOIN dvi_hotspot_place c ON c.hotspot_ID = bm.between_hotspot_id
    WHERE bm.between_hotspot_id = ${poovar.id}
      AND (${pairConditions})
...
async function main() {
  const resolved = await resolveHotspotTestIdsSouth();
  ...
  const poovarRows = await poovarFocusedQuery(slots, candidates);
  ...
  console.log('\nPoovar Focused Rows:');
  console.log(JSON.stringify(poovarRows, null, 2));
}

main()
  .catch((error) => {
    console.error('Failed running south hotspot fit test matrix:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
