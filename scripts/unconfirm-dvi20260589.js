...
async function countRows(tx) {
  return {
    ...
    confirmedPlan: await tx.dvi_confirmed_itinerary_plan_details.count({
      where: { confirmed_itinerary_plan_ID: CONFIRMED_PLAN_ID, itinerary_plan_ID: PLAN_ID },
    }),
    customers: await tx.dvi_confirmed_itinerary_customer_details.count({
      where: { confirmed_itinerary_plan_ID: CONFIRMED_PLAN_ID },
    }),
...
async function main() {
  console.log('[UNCONFIRM] Starting for', { QUOTE_ID, PLAN_ID, CONFIRMED_PLAN_ID, DRY_RUN });
  ...
  await prisma.$transaction(async (tx) => {
    const confirmedPlan = await tx.dvi_confirmed_itinerary_plan_details.findFirst({
      where: {
        confirmed_itinerary_plan_ID: CONFIRMED_PLAN_ID,
