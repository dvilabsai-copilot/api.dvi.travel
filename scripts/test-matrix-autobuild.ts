/*
  Simple integration test: POST a plan to trigger build and GET details.
  Usage: HOTSPOT_MATRIX_AUTOBUILD=true BASE_URL=http://localhost:4006 tsx scripts/test-matrix-autobuild.ts
*/
...
async function httpRequest({ method, path, body, headers = {} }: any) {
...
async function main() {
  console.log('POSTing plan to trigger build...');
  const post = await httpRequest({ method: 'POST', path: '/api/v1/itineraries/?type=itineary_basic_info', body: planPayload });
  console.log('POST result:', post.status, post.ok);
  if (post.json) console.log('POST json:', JSON.stringify(post.json).slice(0, 500));

  console.log('GET details for DVI20260589...');
  const get = await httpRequest({ method: 'GET', path: '/api/v1/itineraries/details/DVI20260589' });
  console.log('GET result:', get.status, get.ok);
