...
/**
 * Copies legacy monthly Offline and AxisRooms prices into the canonical dated
 * occupancy-rate table. This script is intentionally dry-run by default.
 *
 * Usage:
...
const axisroomsOnly = args.has("--axisrooms-only");
const hotelArgIndex = process.argv.indexOf("--hotel-id");
const requestedHotelId =
  hotelArgIndex >= 0 ? Number(process.argv[hotelArgIndex + 1]) : 0;
...
function dateFor(year: unknown, month: unknown, day: number): Date | null {
  ...
  return date.getUTCMonth() === monthNumber &&
...
function compositeKey(
  candidate: Pick<Candidate, "hotelId" | "roomId" | "rateplanId" | "date">,
): string {
...
async function main() {
  if (args.has("--help")) {
    console.log(
      "Dry run: npx tsx scripts/migrate-room-pricebook-to-occupancy-rates.ts",
    );
    console.log(
      "Apply:   npx tsx scripts/migrate-room-pricebook-to-occupancy-rates.ts --apply [--hotel-id ID] [--batch-size N]",
    );
    console.log(
      "Filters: add --offline-only or --axisrooms-only for a provider-specific run",
    );
    return;
  }
  if (
    hotelArgIndex >= 0 &&
    (!Number.isInteger(requestedHotelId) || requestedHotelId <= 0)
  ) {
    throw new Error("--hotel-id must be a positive integer");
  }
  if (offlineOnly && axisroomsOnly)
    throw new Error("Use only one of --offline-only or --axisrooms-only");

  // The legacy pricebook is shared by Offline and AxisRooms hotels. Both
  // providers must be consolidated into the canonical occupancy table.
  const eligibleHotels = await prisma.$queryRawUnsafe<
    Array<{ hotel_id: number; axisrooms_enabled: number | null }>
  >(
    `SELECT hotel_id, axisrooms_enabled FROM dvi_hotel
     WHERE status = 1
       ${offlineOnly ? "AND (axisrooms_enabled IS NULL OR axisrooms_enabled <> 1)" : ""}
       ${axisroomsOnly ? "AND axisrooms_enabled = 1" : ""}
       ${requestedHotelId > 0 ? `AND hotel_id = ${requestedHotelId}` : ""}`,
  );
  const hotelProvider = new Map(
    eligibleHotels.map((hotel) => [
      Number(hotel.hotel_id),
      Number(hotel.axisrooms_enabled) === 1 ? "axisrooms" : "offline",
    ]),
  );
  const hotelIds = eligibleHotels
    .map((hotel) => Number(hotel.hotel_id))
    .filter((id) => id > 0);
  if (hotelIds.length === 0) throw new Error("No matching hotels found");

  const legacy = await (prisma as any).dvi_hotel_room_price_book.findMany({
    where: { status: 1, deleted: 0, hotel_id: { in: hotelIds } },
    select: {
      hotel_id: true,
      room_id: true,
      price_type: true,
      ...
      month: true,
      ...Object.fromEntries(
        Array.from({ length: 31 }, (_, i) => [`day_${i + 1}`, true]),
      ),
    },
  });
  ...
  const legacyRoomKeys = Array.from(
    new Set(
      legacy.map(
        (row: any) => `${Number(row.hotel_id)}|${Number(row.room_id)}`,
      ),
    ),
  );
  const existingPlanKeys = new Set(
    planRows.map(
      (plan: any) => `${Number(plan.hotel_id)}|${Number(plan.room_id)}`,
    ),
  );
  const roomsWithoutPlans = legacyRoomKeys.filter(
    (key) => !existingPlanKeys.has(key),
  );
  if (roomsWithoutPlans.length > 0) {
    console.log(
      `Rooms without configured rate plans: ${roomsWithoutPlans.length}. ${apply ? "Creating CP_PLAN fallback rows." : "Dry-run only."}`,
    );
    if (apply) {
      const roomIds = roomsWithoutPlans
  ...
  for (const row of legacy) {
    ...
    const plans =
      plansByRoom.get(`${Number(row.hotel_id)}|${Number(row.room_id)}`) || [];
    if (!plans.length) {
      skippedRows++;
      continue;
    }
    for (let day = 1; day <= 31; day++) {
      ...
      for (const plan of plans) {
        const key = compositeKey({
          ...
          rateplanId: String(plan.rateplan_id),
          date,
        });
    ...
    JSON.stringify(
      {
        ...
        offlineHotels: hotelIds.filter(
          (id) => hotelProvider.get(id) === "offline",
        ).length,
        axisroomsHotels: hotelIds.filter(
          (id) => hotelProvider.get(id) === "axisrooms",
        ).length,
  ...
  for (let offset = 0; offset < candidateRows.length; offset += batchSize) {
    ...
    for (const candidate of batch) {
      ...
      if (!existing) {
        creates.push({
          ...
          source:
            hotelProvider.get(candidate.hotelId) === "axisrooms"
              ? "axisrooms_legacy"
              : "manual_legacy",
        });
      } else if (changed) {
    ...
    if (creates.length > 0) {
      await (prisma as any).dvi_hotel_occupancy_rate.createMany({
        data: creates,
        skipDuplicates: true,
      });
    ...
    if (updates.length > 0) {
      await prisma.$transaction(
        updates.map((item) =>
          (prisma as any).dvi_hotel_occupancy_rate.update({
            where: { id: item.id },
            data: {
              occupancy_rates: item.occupancyRates,
              source: item.source,
    ...
    console.log(
      `Progress: ${Math.min(offset + batch.length, candidateRows.length)}/${candidateRows.length}`,
    );
  }
