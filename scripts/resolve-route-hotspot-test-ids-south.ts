...
const ROUTE_SLOTS: RouteSlotInput[] = [
  { from: 'Marine Drive - Cochin', to: 'Kovalam Beach' },
  { from: 'Kovalam Beach', to: 'Sree Padmanabhaswamy Temple' },
  { from: 'Sree Padmanabhaswamy Temple', to: 'Sunset Point, Kanyakumari' },
];
...
const CANDIDATES = [
  ...
  'Thanumalayan Temple (Suchindram Temple)',
  'Ferry Ride to Vivekananda Rock & Tiruvalluvar Statue and Gandhi Mandapam',
  'Kovalam Beach',
...
export async function resolveHotspotTestIdsSouth(): Promise<ResolveResult> {
  const namesToResolve = new Set<string>();
  const optionalOnlyNames = new Set<string>();
...
async function main() {
  const result = await resolveHotspotTestIdsSouth();
  ...
  if (result.unresolvedRequiredNames.length || result.unresolvedOptionalNames.length) {
    ...
    for (const name of result.unresolvedOptionalNames) {
      console.log(`- ${name}`);
    }
  }
...
main()
  .catch((error) => {
    console.error('Failed resolving south route hotspot test IDs:', error);
    process.exitCode = 1;
  })
  .finally(async () => {
