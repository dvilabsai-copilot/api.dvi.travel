...
type BetweenRejectionRow = {
  ...
  betweenHotspotId: number;
  betweenHotspotName: string;
  routeFitType: RouteFitType | null;
  rejectionCode: 'OFF_ROUTE_SKIPPED' | 'LEG_BUILD_FAILED';
  rejectionReason: string | null;
  candidateDistanceFromAbRouteMeters: number | null;
  roadDetourKm: number | null;
  roadDetourRatio: number | null;
...
type RunSummary = {
  ...
  betweenSkippedExisting: number;
  matrixLegsBuiltForBetween: number;
  betweenRowsInserted: number;
  betweenRowsRejected: number;
  offRouteRowsRejected: number;
  legFailedRowsRejected: number;
...
function usage(): void {
  ...
  console.log('  --max-haversine-km <n>              Default: 20000 (all India coverage)');
  console.log('  --max-candidate-haversine-km <n>    Default: 20000 (all India coverage)');
...
function normalizeArgs(raw: RawArgs): InputArgs {
  ...
  const maxInsertDetourRatio = toNumberOrDefault(process.env.MAX_INSERT_DETOUR_RATIO, 0.5);
...
async function ensureHelperTablesForDb(targetDb: string): Promise<void> {
  ...
  await prisma.$executeRawUnsafe(`
    CREATE TABLE IF NOT EXISTS \`${targetDb}\`.\`hotspot_route_between_rejections\` (
      id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
      from_hotspot_id INT NOT NULL,
      from_hotspot_name TEXT NULL,
      from_hotspot_location TEXT NULL,
      ...
      between_hotspot_id INT NOT NULL,
      between_hotspot_name TEXT NULL,
      route_fit_type VARCHAR(40) NULL,
      ...
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
      PRIMARY KEY (id),
      UNIQUE KEY uniq_hotspot_route_between_rejection (from_hotspot_id, to_hotspot_id, between_hotspot_id)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
  `);
...
async function fetchLocationHotspots(args: InputArgs): Promise<{
  ...
  for (const row of rows) {
    ...
    if (lat === null || lng === null || !isValidLatLng(lat, lng)) {
      invalid += 1;
      console.warn(
        `SKIP INVALID COORD hotspot_ID=${id}, lat=${row.hotspot_latitude}, lng=${row.hotspot_longitude}`,
      );
      continue;
    }
...
function pairKey(aId: number, bId: number): string {
  const first = Math.min(aId, bId);
  const second = Math.max(aId, bId);
  return `${first}:${second}`;
}

function canonicalizeEndpoints(a: Hotspot, b: Hotspot): EndpointPair {
  if (a.id <= b.id) {
    return { from: a, to: b };
  }
  return { from: b, to: a };
...
function canonicalizeBetweenRow(row: BetweenRow): BetweenRow {
  if (row.fromHotspotId <= row.toHotspotId) {
    return row;
  }

  return {
    ...row,
    fromHotspotId: row.toHotspotId,
    fromHotspotName: row.toHotspotName,
    fromHotspotLocation: row.toHotspotLocation,
    toHotspotId: row.fromHotspotId,
    toHotspotName: row.fromHotspotName,
    toHotspotLocation: row.fromHotspotLocation,
  };
}

function canonicalizeBetweenRejectionRow(row: BetweenRejectionRow): BetweenRejectionRow {
  if (row.fromHotspotId <= row.toHotspotId) {
    return row;
  }
...
async function getBetweenMapRowExistsForDb(
  targetDb: string,
  fromHotspotId: number,
  toHotspotId: number,
  betweenHotspotId: number,
): Promise<boolean> {
  const canonFrom = Math.min(fromHotspotId, toHotspotId);
  const canonTo = Math.max(fromHotspotId, toHotspotId);
...
async function upsertRouteMatrixStatusForDb(
  ...
  console.log('Inserted / updated matrix row for', from.id, '->', to.id, 'status:', payload.processStatus);
}
...
async function upsertBetweenMapRowForDb(targetDb: string, row: BetweenRow): Promise<void> {
  const canonicalRow = canonicalizeBetweenRow(row);

  await prisma.$executeRawUnsafe(
    `
      DELETE FROM \`${targetDb}\`.\`hotspot_route_between_map\`
    ...
    `,
    canonicalRow.betweenHotspotId,
    canonicalRow.fromHotspotId,
    canonicalRow.toHotspotId,
    canonicalRow.toHotspotId,
    canonicalRow.fromHotspotId,
  );
  ...
  await prisma.$executeRawUnsafe(
    ...
    `,
    canonicalRow.fromHotspotId,
    canonicalRow.fromHotspotName,
    canonicalRow.fromHotspotLocation,
    canonicalRow.toHotspotId,
    canonicalRow.toHotspotName,
    canonicalRow.toHotspotLocation,
    canonicalRow.betweenHotspotId,
    canonicalRow.betweenHotspotName,
    canonicalRow.distanceFromRouteMeters,
    canonicalRow.candidateDistanceFromAbRouteMeters,
    canonicalRow.candidateProgressOnAbRatio,
    canonicalRow.destinationDistanceFromAcRouteMeters,
    canonicalRow.destinationProgressOnAcRatio,
    canonicalRow.crossesDestinationBeforeCandidate ? 1 : 0,
    canonicalRow.abOsrmDistanceKm,
    canonicalRow.acOsrmDistanceKm,
    canonicalRow.cbOsrmDistanceKm,
    canonicalRow.insertedRouteDistanceKm,
    canonicalRow.roadDetourKm,
    canonicalRow.roadDetourRatio,
    canonicalRow.routeDecisionReason,
    canonicalRow.routeFitType,
  );
}

async function upsertBetweenRejectionForDb(targetDb: string, row: BetweenRejectionRow): Promise<void> {
  const canonicalRow = canonicalizeBetweenRejectionRow(row);

  await prisma.$executeRawUnsafe(
    `
      INSERT INTO \`${targetDb}\`.\`hotspot_route_between_rejections\` (
        from_hotspot_id,
        from_hotspot_name,
        from_hotspot_location,
        to_hotspot_id,
    ...
    canonicalRow.betweenHotspotName,
    canonicalRow.routeFitType,
    canonicalRow.rejectionCode,
    canonicalRow.rejectionReason,
    canonicalRow.candidateDistanceFromAbRouteMeters,
    canonicalRow.roadDetourKm,
    canonicalRow.roadDetourRatio,
    canonicalRow.errorMessage,
  );
}
...
function initializeSummary(args: InputArgs): RunSummary {
  return {
    ...
    betweenSkippedExisting: 0,
    matrixLegsBuiltForBetween: 0,
    betweenRowsInserted: 0,
    betweenRowsRejected: 0,
    offRouteRowsRejected: 0,
    legFailedRowsRejected: 0,
...
async function runDirectMatrixBuild(
  ...
  for (const from of sourceHotspots) {
    if (stop) break;

    for (const to of destinationHotspots) {
      ...
      if (existing) {
        ...
        console.log(
          `directPairsSkippedExistingDone ${from.id}->${to.id}`,
        );
        continue;
      }
...
async function runBetweenMapBuild(
  args: InputArgs,
  sourceHotspots: Hotspot[],
  destinationHotspots: Hotspot[],
  candidateHotspots: Hotspot[],
  summary: RunSummary,
): Promise<void> {
  ...
  for (const source of sourceHotspots) {
    if (stop) break;

    for (const destination of destinationHotspots) {
      if (source.id === destination.id) continue;
      if (stop) break;

      const endpointKey = pairKey(source.id, destination.id);
      if (processedPairs.has(endpointKey)) {
        continue;
      }
      processedPairs.add(endpointKey);

      const endpoint = canonicalizeEndpoints(source, destination);
      const from = endpoint.from;
      const to = endpoint.to;

      let ab = await getDoneMatrixRowForDb(args.targetDb, from.id, to.id);
      if ((!ab || ab.coordinates.length < 2) && args.apply) {
        try {
          ab = await ensureRouteLegForDb(args.targetDb, from, to, args, summary, true, true);
        } catch {
          ab = null;
        }
      }
      ...
      if (!ab || ab.coordinates.length < 2) {
        ...
        console.warn(`BETWEEN SKIP missing AB route ${from.id}->${to.id}`);
        continue;
      }
      ...
      for (const candidate of candidateHotspots) {
        ...
        if (acHaversine > args.maxCandidateHaversineKm || cbHaversine > args.maxCandidateHaversineKm) {
          ...
          continue;
                ...
                if (!args.forceBetween) {
          const exists = await getBetweenMapRowExistsForDb(args.targetDb, from.id, to.id, candidate.id);
          if (exists) {
            summary.betweenSkippedExisting += 1;
            console.log(
              `BETWEEN SKIP EXISTING ${from.id}->${candidate.id}->${to.id} (${from.name} -> ${candidate.name} -> ${to.name}) already in hotspot_route_between_map`,
            );
            continue;
          }
        }
        ...
        try {
          ...
          if (!args.apply) {
            ...
            await upsertBetweenRejectionForDb(args.targetDb, {
              ...
              routeFitType: null,
              rejectionCode: 'LEG_BUILD_FAILED',
            ...
            summary.betweenRowsRejected += 1;
            summary.legFailedRowsRejected += 1;
            summary.failed += 1;
          }
          continue;
        }
        ...
        if (routeFitType === 'OFF_ROUTE' && !args.includeOffRoute) {
          if (args.apply) {
            await upsertBetweenRejectionForDb(args.targetDb, {
              ...
              betweenHotspotName: candidate.name,
              routeFitType,
              rejectionCode: 'OFF_ROUTE_SKIPPED',
              rejectionReason: routeDecisionReason,
          ...
          summary.betweenRowsRejected += 1;
          summary.offRouteRowsRejected += 1;
          continue;
        }
        ...
        console.log(
          `BETWEEN INSERT ${from.id}->${candidate.id}->${to.id} (${from.name} -> ${candidate.name} -> ${to.name}) fit=${routeFitType} detour=${row.roadDetourKm.toFixed(3)}km ratio=${row.roadDetourRatio.toFixed(4)}`,
        );
...
async function main(): Promise<void> {
  ...
  console.log(`total all-hotspot matches: ${allHotspotsResult.allMatches}`);
  console.log(`total valid all-hotspots: ${allHotspotsResult.validHotspots.length}`);
  console.log(`total invalid all-hotspots: ${allHotspotsResult.invalidCount}`);
  ...
  if (!allHotspotsResult.validHotspots.length) {
    console.log('No valid hotspots found in dvi_hotspot_place. Nothing to process.');
    console.log(JSON.stringify(summary, null, 2));
    return;
  }
