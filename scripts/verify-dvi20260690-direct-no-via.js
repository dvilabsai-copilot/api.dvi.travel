...
function verifyDay2(details) {
  ...
  const forbidden = attractionNames.filter((name) =>
    /Santa Cruz|Kerala Folklore|Athirapp/i.test(name),
  );
  ...
  if (forbidden.length > 0) {
    return {
      ok: false,
      reason: `Day 2 contains forbidden source/via attractions for direct route: ${forbidden.join(', ')}`,
      day2,
    };
  }

  if (!hasCheeyappara || !hasValara) {
    return {
      ok: false,
      reason: `Day 2 is missing expected direct-route corridor attractions. Found=${attractionNames.join(', ')}`,
      day2,
    };
  }

  if (!(cheeyapparaIndex < valaraIndex)) {
    return {
      ok: false,
      reason: `Day 2 attraction order is wrong. Expected Cheeyappara -> Valara, found=${attractionNames.join(' -> ')}`,
      day2,
    };
  }

  return { ok: true, reason: 'Day 2 contains Cheeyappara and Valara without source-city leakage', day2 };
}
...
async function main() {
  ...
  console.log('[verify-dvi20260690-direct-no-via] POST', buildUrl);
  ...
  console.log('[verify-dvi20260690-direct-no-via] build status', buildResult.response.status);
  ...
  if (!quoteId) {
    console.error('[verify-dvi20260690-direct-no-via] missing quoteId in build response');
    process.exit(1);
  }
  ...
  for (let attempt = 1; attempt <= 5; attempt++) {
    console.log('[verify-dvi20260690-direct-no-via] GET attempt', attempt, detailsUrl);
    detailsResult = await fetchJson(detailsUrl, {
      method: 'GET',
      headers: commonHeaders,
    });

    console.log('[verify-dvi20260690-direct-no-via] details status', detailsResult.response.status);
    ...
    console.log('[verify-dvi20260690-direct-no-via] Day 2 summary', JSON.stringify(summary, null, 2));

    if (verification.ok) {
      console.log('[verify-dvi20260690-direct-no-via] verification passed:', verification.reason);
      return;
    }

    if (attempt < 5) {
      console.log('[verify-dvi20260690-direct-no-via] waiting before retry:', verification.reason);
      await sleep(1500);
    }
  }

  const finalVerification = verifyDay2(detailsResult?.data);
  console.error('[verify-dvi20260690-direct-no-via] verification failed:', finalVerification.reason);
  process.exit(1);
}

main().catch((error) => {
  console.error('[verify-dvi20260690-direct-no-via] fatal', error?.stack || error?.message || String(error));
  process.exit(1);
});
