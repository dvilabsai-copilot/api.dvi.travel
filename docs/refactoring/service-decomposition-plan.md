...
| `timeline.builder.ts` | 8,696 lines / 28 DB-call matches | route processing; hotspot selection; travel legs; operating hours/cutoffs | pure timeline policy tests -> candidate policy -> input loader -> Day-1 fallback -> policy services | ordering/timing parity |
