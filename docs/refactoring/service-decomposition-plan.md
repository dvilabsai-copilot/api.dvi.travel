...
| `timeline.builder.ts` | 7,453 lines / 21 DB-call matches | route processing; travel legs; operating hours/cutoffs | pure timeline policy tests -> candidate policy -> input loader -> Day-1 fallback -> route-hotspot selection -> arrival/hotel decisions -> hotel-first insertion -> non-hotel cutoff -> policy services | ordering/timing parity |
