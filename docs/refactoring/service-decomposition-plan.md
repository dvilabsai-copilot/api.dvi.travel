...
|---|---:|---|---|---|
| `itineraries.service.ts` | 31,061 lines / 613 DB-call matches | create/update persistence; route/rebuild; manual/auto fit; hotel/vehicle/vendor pricing; confirmation/cancellation | guide assignment -> vehicle status -> vehicle orchestration -> plan persistence -> activity -> smart activity -> hotspot workflow -> selection workflow -> quote context -> confirmation -> manual preview boundary -> route/rebuild -> pricing | transaction and shared mutable state |
