...
| `timeline.builder.ts` | 6,516 lines / 10 DB-call matches | route processing; travel legs; operating hours/cutoffs | pure timeline policy tests -> candidate policy -> input loader -> Day-1 fallback -> route-hotspot selection -> route-hotspot planning -> manual/same-city placement ordering -> destination reservation -> carry-forward attachment -> matrix autobuild -> candidate reordering -> arrival/hotel decisions -> hotel-first insertion -> non-hotel cutoff -> policy services | ordering/timing parity |
