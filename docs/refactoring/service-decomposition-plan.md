...
`ItineraryGuideAssignmentWriteService` now owns draft guide-assignment validation, eligible-guide cost resolution, guide-row upsert and per-route/all-itinerary slot-cost persistence. `ItinerariesService` remains the compatibility facade and delegates the existing controller contract; the existing guide policy service owns availability and cost rules. Transaction ordering, payload normalization and response projection are unchanged; guide-cost query latency and slot-row volume remain pending measurement.
