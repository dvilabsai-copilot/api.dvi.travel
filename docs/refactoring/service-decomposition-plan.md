...
|---|---:|---|---|---|
| `itineraries.service.ts` | 32,712 lines / 613 DB-call matches | create/update persistence; route/rebuild; manual/auto fit; hotel/vehicle/vendor pricing; confirmation/cancellation | guide assignment -> vehicle status -> vehicle orchestration -> plan persistence -> activity -> smart activity -> hotspot workflow -> manual preview boundary -> route/rebuild -> pricing -> confirmation | transaction and shared mutable state |
