...
| `timeline.builder.ts` | 7,826 lines / 21 DB-call matches | route processing; travel legs; operating hours/cutoffs | pure timeline policy tests -> candidate policy -> input loader -> Day-1 fallback -> route-hotspot selection -> policy services | ordering/timing parity |
